...
# Cosmos-NeMo-Assets dataset and dataloader for LoRA training (shared between video2world and text2world)
...
# Text2World LoRA post-training configuration for 2B model using video2world architecture
# torchrun --nproc_per_node=8 --master_port=12341 -m scripts.train --config=cosmos_predict2/_src/predict2/configs/video2world/config.py -- experiment=predict2_text2world_lora_training_2b_cosmos_nemo_assets
predict2_text2world_lora_training_2b_cosmos_nemo_assets = dict(
    defaults=[
        f"/experiment/{DEFAULT_CHECKPOINT.experiment}",
        {"override /data_train": "mock"},
        {"override /data_val": "mock"},
    ...
    job=dict(
        project="cosmos_predict_v2p5",
        group="text2world_lora",
        name="2b_cosmos_nemo_assets_text2world_lora",
    ),
    dataloader_train=dataloader_train_cosmos_nemo_assets_lora,
    checkpoint=dict(
        save_iter=200,
...
for _item in [
    ...
    predict2_text2world_lora_training_2b_cosmos_nemo_assets,
]:
